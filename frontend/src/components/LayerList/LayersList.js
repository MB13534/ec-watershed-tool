...
const LayerListItem = ({ name, layerName, geometryType, enabled, handleLayerToggle }) => {
    ...
    <div className={classes.root}>
      ...
      <List dense>
        {items.length === 0 ? (
          ...
          items.filter(x => !x.hideFromLegend).sort((a, b) => (a.legendOrder > b.legendOrder) ? 1 : -1).map((item, i) => (
            <LayerListItem
              name={item.name}
